...
export default function NotFoundPage({ children }: NotFoundProps) {
  ...
  return (
    <div
      ref={pageRef}
      className="min-h-screen bg-gradient-to-br from-black-750 via-black to-yellow-950 flex items-center justify-center p-4 relative overflow-hidden"
    >
      ...
      >
        ...
        <div ref={textRef} className="mb-12">
          <h1
            className={`text-3xl md:text-4xl font-bold text-gray-500 mb-4 ${styles.glow_text}`}
          >
            Ой! Сторінку не знайдено
          </h1>
          <p className="text-xl text-gray-600 mb-6 max-w-md mx-auto">
            Схоже, що сторінка, яку ви шукаєте, переїхала або тимчасово
            недоступна.
          </p>
        ...
        <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <Link
            href="/services"
            className={`text-gray-500 hover:text-amber-600 transition-colors ${styles.gradient_border}`}
          >
            Послуги
          </Link>
          <Link
