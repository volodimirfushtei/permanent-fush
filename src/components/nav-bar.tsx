...
export default function NavBar({ children }: NavBarProps) {
  ...
  return (
      <>
          ...
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between h-20 md:h-24">
              ...
              <span className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-500 bg-clip-text text-transparent">
                ...
                <div className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-amber-400 to-yellow-300 transition-all duration-300 group-hover:w-full pointer-events-none"></div>
              </Link>
                    ...
                    { label: "Contacts", path: "/contacts" },
                      ...
                      >
                        ...
                        >
                          {item.label}
                          <div className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-amber-400 to-yellow-300 transition-all duration-300 group-hover:w-full pointer-events-none"></div>
                          <div className="absolute inset-0 bg-gradient-to-r from-amber-400/5 to-yellow-300/5 opacity-0 group-hover:opacity-100 rounded-lg transition-opacity duration-300 pointer-events-none"></div>
                        </Link>
                      </li>
                  ))}
                </ul>
              ...
              <div className="flex items-center space-x-4">
                <button
                    ref={buttonRef}
                    className="hidden md:flex items-center space-x-2 bg-gradient-to-r from-amber-500/10 to-yellow-400/10 text-amber-300 text-sm font-medium px-4 py-2 rounded-full border border-amber-500/20 hover:bg-amber-500/20 hover:border-amber-400/40 hover:text-white transition-all duration-300 group relative"
                >
                  <span className="w-2 h-2 bg-amber-400 rounded-full animate-pulse pointer-events-none"></span>
                ...
                <button
                    className="md:hidden flex flex-col justify-center w-10 h-10 space-y-1.5 group p-2 z-50 relative"
                    onClick={() => setIsMenuOpen(!isMenuOpen)}
                    aria-label="Toggle menu"
                >
                <span
                    className={`block w-6 h-0.5 bg-amber-400 transition-all duration-300 transform ${
                        isMenuOpen ? "rotate-45 translate-y-2 bg-white" : "group-hover:bg-white"
                    }`}
                ></span>
                  <span
                      className={`block w-6 h-0.5 bg-amber-400 transition-all duration-300 transform ${
                          isMenuOpen ? "-rotate-45 -translate-y-0.5 bg-white" : "group-hover:bg-white"
                      }`}
                  ></span>
                </button>
              </div>
        ...
        <div
            ref={mobileRef}
            className={`fixed top-0 left-0 w-full h-screen bg-gradient-to-br from-black to-gray-900 backdrop-blur-2xl z-40 md:hidden ${
                ...
                {[
                  ...
                  { label: "Contacts", path: "/contacts" },
                    ...
                    <li key={item.label} className="relative group">
                      <Link
                          href={item.path}
                          className="text-2xl font-semibold text-white hover:text-amber-400 transition-colors duration-300 flex items-center py-3"
                          onClick={() => {
                            setIsMenuOpen(false);
                          }}
                      >
            ...
            <div className="mt-12 pt-8 border-t border-white/10">
              <div className="space-y-4">
                <a
                    ...
                    className="flex items-center text-amber-400 text-lg font-semibold py-3"
                >
                ...
                <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center text-white text-lg opacity-80 hover:opacity-100 transition-opacity py-3"
                >
